...
static InterpretResult run() {
	...
	#define READ_BYTE() (*frame->ip++) //macro for read byte of the next IP to be run
	#define READ_CONSTANT() (frame->closure->function->chunk.constants.values[READ_BYTE()]) //macro to read out constant within chunk at byte
	#define READ_SHORT() (frame->ip += 2, (uint8_t)((frame->ip[-2] << 8) | frame-> ip[-1])) //takes the next two bytes from chunk and build a 16-bit unsigned integer
			...
			switch (instruction = READ_BYTE()) {
				...
				case OP_ADD: {
					if (IS_STRING(peek(0)) && IS_STRING(peek(1))) {
						...
						double a = AS_NUMBER(pop());
						push(NUMBER_VAL(a + b));
					} else { //TODO: add support for concatenate anything, just convert it to string for each type
						runtimeError(
